  ...
  try {
    ...
    const postDraft = postDraftInput
      ? {
          ...
          authorProfileId: String(postDraftInput.authorProfileId || decoded.uid || '').trim(),
          authorOwnerUid: String(postDraftInput.authorOwnerUid || decoded.uid || '').trim(),
      ...
      await db.runTransaction(async (transaction) => {
        ...
        if (!postSnap.exists) {
          transaction.create(postRef, {
            ...
            authorId: userId,
            authorUid: userId,
            authorProfileId: latestUpload?.profileId || postDraft?.authorProfileId || userId,
            authorOwnerUid: latestUpload?.ownerUid || postDraft?.authorOwnerUid || userId,
