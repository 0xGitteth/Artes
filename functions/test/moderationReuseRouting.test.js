...
test('only successful contract-validated caches from the active Gemini prompt are reusable', () => {
  ...
  assert.equal(isReusableModerationCache({ geminiDiagnostics: { promptVersion: 'gemini_moderation_v2', success: true, contractValidated: true, fallbackUsed: false, safetyBlocked: false } }, 'gemini_moderation_v2'), true);
});
...
test('cache-hit diagnostics preserve verified provenance without claiming a fresh Gemini call', () => {
  ...
  assert.equal(isReusableModerationCache({ geminiDiagnostics: reused }, 'gemini_moderation_v2'), true);
