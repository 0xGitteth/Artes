...
const BATCHES = {
  balanced: {
    key: 'balanced',
    title: 'Balanced target scene review',
    imageDir: 'balanced-target-scene-previews',
    manifestFile: 'balanced-target-scene-preview-screening.json',
    outputFile: 'balanced-target-scene-labels.reviewed.json',
    prefillFile: 'moderation-balanced-target-scene-assistant-prefill-v1.json',
    manifestStatus: 'research_balanced_professional_adult_target_scene_preview_screening_only',
    prefillVersion: 'balanced_target_scene_assistant_prefill_v1',
    itemCount: 78,
    uniquePoolCount: 26,
    defaultPort: 8794,
    defaultBasis: 'assistant_visual_review_from_user_uploaded_balanced_target_scene_review_batch',
  },
  'scene-depth': {
    key: 'scene-depth',
    title: 'AdultLabs scene depth review',
    imageDir: 'adultlabs-scene-depth-previews',
    manifestFile: 'adultlabs-scene-depth-preview-screening.json',
    outputFile: 'adultlabs-scene-depth-labels.reviewed.json',
    prefillFile: 'moderation-adultlabs-scene-depth-assistant-prefill-v1.json',
    manifestStatus: 'research_adultlabs_scene_depth_preview_screening_only',
    prefillVersion: 'adultlabs_scene_depth_assistant_prefill_v1',
    itemCount: 63,
    uniquePoolCount: 21,
    defaultPort: 8795,
    defaultBasis: 'assistant_visual_review_from_user_uploaded_adultlabs_scene_depth_review_batch',
  },
};

const BATCH_KEY = process.env.ARTES_TARGET_SCENE_REVIEW_BATCH || 'balanced';
const BATCH = BATCHES[BATCH_KEY];
if (!BATCH) throw new Error(`unknown_target_scene_review_batch:${BATCH_KEY}`);

const IMAGE_DIR = path.join(ROOT, BATCH.imageDir);
const MANIFEST_PATH = path.join(ROOT, BATCH.manifestFile);
const OUTPUT_PATH = path.join(ROOT, BATCH.outputFile);
const PREFILL_PATH = path.join(REPO_ROOT, 'docs', BATCH.prefillFile);
const PORT = Number(process.env.ARTES_TARGET_SCENE_REVIEW_PORT || BATCH.defaultPort);
...
const validateManifest = (manifest) => {
  if (
    manifest?.status !== BATCH.manifestStatus
    || manifest?.fetchedCount !== BATCH.itemCount || manifest?.failedCount !== 0 || manifest?.uniquePoolCount !== BATCH.uniquePoolCount
  ...
  ) throw new Error(`target_scene_review_manifest_not_ready:${BATCH.key}`);
};
...
const validateLabel = (label) => {
  ...
  if (label.graphicInjury !== 'none') throw new Error('target_scene_graphic_injury_must_be_none');
  if (!Array.isArray(label.sensitiveSignals) || label.sensitiveSignals.length !== 0) throw new Error('target_scene_sensitive_signals_must_be_empty');
...
const expandPrefill = (prefill, manifest) => {
  if (
    prefill?.prefillVersion !== BATCH.prefillVersion
    || prefill?.suggestionSource !== 'assistant_visual_review'
    || prefill?.authoritative !== false || prefill?.humanConfirmationRequired !== true
    || prefill?.discoveryMetadataUsedAsLabelAuthority !== false || prefill?.itemCount !== BATCH.itemCount
    || !Array.isArray(prefill?.indexRules)
  ) throw new Error(`target_scene_prefill_invalid:${BATCH.key}`);
  const byIndex = new Map();
  for (const rule of prefill.indexRules) {
    if (!ELIGIBILITY_DECISIONS.includes(rule.researchEligibilityDecision)) throw new Error('target_scene_prefill_eligibility_invalid');
    for (const index of rule.indices || []) {
      if (byIndex.has(index)) throw new Error(`target_scene_prefill_duplicate_index:${index}`);
      const suggestion = {
        ...
        basis: rule.basis || BATCH.defaultBasis,
      };
      if (suggestion.researchEligibilityDecision === 'include_real_photograph') {
        if (!AGE_SAFETY_DECISIONS.includes(suggestion.ageSafetyDecision)) throw new Error(`target_scene_prefill_age_invalid:${index}`);
        if (suggestion.ageSafetyDecision === 'skip_minor_or_age_uncertain') {
          if (suggestion.detectorLabel !== null) throw new Error(`target_scene_prefill_age_skip_has_label:${index}`);
        } else {
          ...
          if (suggestion.ageSafetyDecision === 'not_required_nonadult_nonsexual' && labelNeedsAgeSafety(suggestion.detectorLabel)) {
            throw new Error(`target_scene_prefill_age_required:${index}`);
          }
        }
      } else if (suggestion.ageSafetyDecision !== null || suggestion.detectorLabel !== null) {
        throw new Error(`target_scene_prefill_exclusion_has_label:${index}`);
      }
      byIndex.set(index, suggestion);
    }
  }
  for (const record of manifest.records) if (!byIndex.has(record.index)) throw new Error(`target_scene_prefill_missing_index:${record.index}`);
  return byIndex;
};
...
const saveReview = async (payload) => {
  ...
  const item = {
    ...
    selectionStrategy: record.selectionStrategy || null, sceneDepthQuantile: record.quantile ?? null, screenshotOrdinal: record.screenshotOrdinal ?? null,
