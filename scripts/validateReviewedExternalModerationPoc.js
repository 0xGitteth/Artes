...
const SOURCES_PATH = path.join(REPO_ROOT, '.tmp', 'moderation-test-images', 'external-poc', 'sources.json');
...
if (intakeItems.length < 2) throw new Error('reviewed_poc_needs_at_least_two_items');
...
for (let i = 0; i < intakeItems.length; i += 1) {
  for (let j = i + 1; j < intakeItems.length; j += 1) {
    ...
    pairs.push({
      ...
      sameGraphicInjury: leftLabel.graphicInjury === rightLabel.graphicInjury,
      sameMinorConcern: leftLabel.possibleMinorConcern === rightLabel.possibleMinorConcern,
    });
  }
}
...
const itemSummaries = intakeItems
  .map((item) => {
...
const roundedMutualPairs = mutualPairs.map((pair) => ({ ...pair, cosineDistance: Number(pair.cosineDistance.toFixed(6)) }));

const output = {
  schemaVersion: 1,
  analysisType: 'external_poc_human_label_validation',
  ...
  items: itemSummaries,
  mutualNearestNeighborPairs: roundedMutualPairs,
...
process.stdout.write(`${JSON.stringify({
  ...
  itemSummaries: output.items.map((item) => ({
    ...
    pocNeighborGroupId: item.pocNeighborGroupId,
  })),
  trainingReady: false,
  thresholdSelected: false,
