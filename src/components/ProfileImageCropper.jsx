...
export default function ProfileImageCropper({ source, measuredHeaderAspectRatio = 3, onApply, onCancel }) {
  ...
  const getScaleLimits = (key) => {
    ...
    const minScale = Math.max(minByWidth, minByHeight, 0.12);
    const maxByWidth = 0.98;
    const maxByHeight = 0.98 * ratio;
    const maxScale = Math.min(maxByWidth, maxByHeight, 0.98);
    return { min: Math.min(minScale, maxScale), max: Math.max(minScale, maxScale) };
  };
